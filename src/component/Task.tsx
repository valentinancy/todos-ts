...
const Task: FunctionComponent<ITaskComponentProps> = props => {
      ...
      if (isOnLeft) {
        actionButtonComponent = (
          <ActionButton
            ...
            buttonType={E_BUTTON_TYPE.ADDING}
          />
        );
      } else {
        if (isAddingTask) {
          actionButtonComponent = (
            <ActionButton
              handleAction={handleAddTask}
              buttonType={E_BUTTON_TYPE.ADD}
            />
          );
        }
      }
    } else {
      actionButtonComponent = isOnLeft ? (
        <ActionButton
          isChecked={isChecked}
          handleAction={handleToggleChecked}
          buttonType={E_BUTTON_TYPE.CHECK}
        />
      ) : (
        <ActionButton
          isChecked={isChecked}
          handleAction={handleDeleteTask}
          buttonType={E_BUTTON_TYPE.DELETE}
        />
      );
    }
  ...
  return (
    <>
      <div
        className={`task-container ${isChecked ? "disabled-container" : ""}`}
      >
        {renderButton(true)}
        <div className="task-details">
          ...
          <p
            id={`task-description${isNewTask ? "-new" : ""}`}
            className={`task-description ${isNewTask ? "disabled" : ""}${
              isAddingTask ? " hide" : ""
            } ${isChecked ? "disabled-text strike-text" : ""}`}
            onClick={
              isNewTask
